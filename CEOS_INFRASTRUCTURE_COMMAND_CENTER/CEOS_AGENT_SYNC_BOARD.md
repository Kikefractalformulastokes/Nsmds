...
| Confirm whether an Airbus relationship exists / may be named in `index.html` | ENRIQUE | DONE | — | Enrique confirmed 2026-07-22: no public Airbus reference, "Aerospace" instead | Closed | None — closed |
| Fix `index.html` malformed markup + Airbus→Aerospace copy | CODE | DONE | Enrique's decision above | `index.html`, `README.md` (this branch); parses cleanly, single `<title>`, script in `<head>`, chart markup in `<body>` | Low | None — committed on `claude/coresyn-ceos-infrastructure-audit-sthqhh`, not deployed to production without approval |
